...
class DiffIoU(nn.Module):
    def __init__(self, dim=100):
        ...
        self.sample_ds_arr = torch.arange(self.max_samples).to(self.device)
    ...
    def line_sum_mask(self, mask, x0, x1, y0, y1, direction):
        out = torch.zeros(mask.size(0)).to(self.device)
    ...
    def self_intersection_iben(self, poly):
        ...
        cross_terms = torch.zeros(poly.size(0)).to(self.device)
        self_terms = torch.zeros(poly.size(0)).to(self.device)
    ...
    def dt_loss(self, poly, gt):
        ...
        gt_dt = torch.from_numpy(np.array(gt_dt)).to(self.device)
        ...
        normal_loss = torch.zeros(poly.size(0)).to(self.device)
