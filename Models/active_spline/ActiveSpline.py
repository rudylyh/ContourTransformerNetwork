...
class ActiveSplineTorch():
    def __init__(self, cp_num, p_num, alpha=0.5, device='cuda'):
        self.cp_num = cp_num - 3
        self.p_num = int(p_num / (self.cp_num))
        self.alpha = alpha
        self.device = device
    ...
    def sample_point(self, cps):
        ...
        # comment to ensure not enclosed contour
...
class ActiveSplineTorchVaryLength():
    def __init__(self, alpha=0.5, device=[0]):
        self.alpha = alpha
        self.device = device
    ...
    def batch_linspace(self, start_t, end_t, step_t):
        ...
        batch_arr = map(torch.linspace, start_t, end_t, step_t)
        batch_arr = [arr.unsqueeze(0) for arr in batch_arr]
        return torch.cat(batch_arr, dim=0).cuda(device=self.gpus[0])
