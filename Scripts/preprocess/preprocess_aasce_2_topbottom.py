...
#
#     # Pick just the first landmark
#     top0 = annos[0]
#     top1 = annos[1]
#     bottom0 = annos[-2]
#     bottom1 = annos[-1]
#
#     new_top_x = (top0[0] + top1[0]) / 2.0
#     new_top_y = (top0[1] + top1[1]) / 2.0
#     new_landmarks['target_control'].append([new_top_x, new_top_y])
#
#     new_bottom_x = (bottom0[0] + bottom1[0]) / 2.0
#     new_bottom_y = (bottom0[1] + bottom1[1]) / 2.0
#     new_landmarks['target_control'].append([new_bottom_x, new_bottom_y])
#
#     initial_controls = {0: [],
#                         1: []}
#     initial_controls[0].append([new_top_x, new_top_y])
#     initial_controls[1].append([new_bottom_x, new_bottom_y])
#
...
#
# with open('aasce_{0}_mean_control_topbottom.json'.format(target_size), 'w') as f:
#     json.dump(mean_controls, f, indent=4)


with open('aasce_{0}_mean_control_topbottom.json'.format(target_size), 'r') as f:
...
#     file_name = each.split('/')[-1]
#     json_file = each + '/' + file_name + '.json'
#     target_root = '../train_val_aasce_{0}_topbottom/train/'.format(target_size)
#
#     img_file = each + '/' + file_name + '.jpg'
#     img = Image.open(img_file)
#     W, H = img.size
...
#     bottom1 = annos[-1]
#
#     new_top_x = (top0[0] * new_W / target_size + top1[0] * new_W / target_size) / 2.0
#     new_top_y = (top0[1] * new_H / target_size + top1[1] * new_H / target_size) / 2.0
#     new_landmarks['target_control'].append([new_top_x, new_top_y])
#
#     new_bottom_x = (bottom0[0] * new_W / target_size + bottom1[0] * new_W / target_size) / 2.0
#     new_bottom_y = (bottom0[1] * new_H / target_size + bottom1[1] * new_H / target_size) / 2.0
#     new_landmarks['target_control'].append([new_bottom_x, new_bottom_y])
#
...
#     file_name = each.split('/')[-1]
#     json_file = each + '/' + file_name + '.json'
#     target_root = '../train_val_aasce_{0}_topbottom/val/'.format(target_size)
#
#     img_file = each + '/' + file_name + '.jpg'
#     img = Image.open(img_file)
#     W, H = img.size
...
for i, each in enumerate(all_test_files):
    ...
    file_name = each.split('/')[-1]
    json_file = each + '/' + file_name + '.json'
    target_root = '../train_val_aasce_{0}_topbottom/test/'.format(target_size)
