...
#
# with open('aasce_{0}_directresize_mean_control.json'.format(target_size), 'w') as f:
#     json.dump(mean_controls, f, indent=4)


with open('aasce_{0}_directresize_mean_control.json'.format(target_size), 'r') as f:
...
#     file_name = each.split('/')[-1]
#     json_file = each + '/' + file_name + '.json'
#     target_root = '../train_val_aasce_{0}_directresize/train/'.format(target_size)
#
#     img_file = each + '/' + file_name + '.jpg'
#     img = Image.open(img_file)
#     W, H = img.size
...
for i, each in enumerate(all_val_files):
    ...
    file_name = each.split('/')[-1]
    json_file = each + '/' + file_name + '.json'
    target_root = '../train_val_aasce_{0}_directresize/val/'.format(target_size)
...
for i, each in enumerate(all_test_files):
    ...
    file_name = each.split('/')[-1]
    json_file = each + '/' + file_name + '.json'
    target_root = '../train_val_aasce_{0}_directresize/test/'.format(target_size)
