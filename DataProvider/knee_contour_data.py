...
class SelfDataset(data.Dataset):
    def __init__(self, mode, opts):
        ...
        pass
        # if self.mode == 'val':
        #     self.data_list = self.data_list[0:48]
    ...
    def __getitem__(self, index):
        ...
        size_scale = np.asarray(self.opts['img_size'], np.float32) / ori_img.shape
        ori_img = cv2.resize(ori_img, (self.opts['img_size'][1], self.opts['img_size'][0]))
        smooth_img = DataUtils.get_smooth_img(ori_img)
        grad_mag = DataUtils.get_gradient_magnitude(smooth_img)

        # Get init contour
        [h,w] = self.opts['img_size']
        # init_ctr_path = os.path.join(self.root, img_name, img_name+'_mean.json')
        # with open(init_ctr_path, 'r') as f:
        #     init_ctr_json = json.load(f)
        # init_control = np.asarray(init_ctr_json[self.opts['roi_name']], dtype=np.float32)
        # init_control = DataUtils.sample_point(init_control, self.opts['cp_num'])
        ...
        # target_control = np.asarray([], dtype=np.float32)
        target_control = np.zeros((1000,2), dtype=np.float32)
        if os.path.exists(target_ctr_path):
            with open(target_ctr_path, 'r') as f:
                target_ctr_json = json.load(f)
            target_control = np.asarray(target_ctr_json[self.opts['roi_name']], dtype=np.float32)
            target_control = (target_control * size_scale).astype(np.float32)
            target_control = DataUtils.sample_arc_point(target_control, self.opts['cp_num'])
        ...
        if self.opts['use_partial_sup'] == True and self.mode == 'train' and img_name != self.opts['one_shot_sample']:
            ...
            if sorted_train_names.index(img_name) < self.opts['sample_percent'] * len(self.data_list):
                ...
                gt_segs = DataUtils.get_partial_target(target_control*[w,h], pred_control, is_circle=False)
                if gt_segs.shape[0] > 0:
                    partial_target[0:gt_segs.shape[0]] = gt_segs # if gt_segs is longer than partial_target, then the pred_control is totally wrong
