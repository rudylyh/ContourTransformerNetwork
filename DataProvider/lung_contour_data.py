...
class SelfDataset(data.Dataset):
    def __init__(self, mode, opts):
        ...
        # if self.mode == 'train':
        #     self.data_list = self.data_list[0:120]
        # pass
    ...
    def __getitem__(self, index):
        ...
        ori_img_path = os.path.join(self.root, img_name, img_name+'_'+self.opts['roi_name']+'.png')
        ...
        init_ctr_path = os.path.join(self.root, img_name, img_name+'_'+self.opts['roi_name']+'_init.json')
        init_ctr_json = json.load(open(init_ctr_path))
        init_control = np.asarray(init_ctr_json['init_ctr'], dtype=np.float32)
        ...
        target_ctr_path = os.path.join(self.root, img_name, img_name+'_'+self.opts['roi_name'] + '_gt.json')
        target_ctr_json = json.load(open(target_ctr_path))
        full_gt_ctr = np.asarray(target_ctr_json['gt_ctr'], dtype=np.float32)
        ...
        partial_target = np.full(init_control.shape, -1.0).astype(np.float32)
        ...
        gcn_component = DataUtils.prepare_gcn_component_lung(self.opts['n_neighbor'], self.opts['cp_num'])
